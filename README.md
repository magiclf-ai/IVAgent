# IVAgent - Multi-Agent 驱动的漏洞挖掘系统

[![Python 3.8+](https://img.shields.io/badge/python-3.8+-blue.svg)](https://www.python.org/downloads/)

IVAgent (Intelligent Vulnerability Analysis Agent) 是一个基于大语言模型(LLM)的智能漏洞挖掘系统，采用 **Agentic 循环** 架构，支持多种分析引擎和二进制/源码分析场景。
...
### 智能编排（核心创新）
- **Agentic 循环**: LLM 自主驱动的 Plan → Act → Review → Append/Stop 循环
- **Workflow 文本规划**: 使用自然语言 Markdown 文档描述分析意图，无需硬编码规则
- **多 Agent 协作**: CodeExplorer + DeepVulnAgent + TaskExecutor 分层协作
- **上下文压缩**: 语义蒸馏 + Tool Call 去重记忆块，支持长时运行

### 可视化
- **Web 界面**: 完整的 Web 界面支持日志追踪、漏洞管理和 Agent 执行可视化

---

## 系统架构

IVAgent 采用 **LLM 驱动的分层架构**，核心是 Agentic 循环：
...
├─────────────────────────────────────────────────────────────────┤
│                      编排调度层                                  │
│  MasterOrchestrator: 协调多 Workflow 执行                        │
│    ↓ Plan → Act → Review → Append/Stop                          │
│  TaskExecutorAgent: 执行任务列表，不负责规划                       │
│  TaskListManager: SQLite 任务看板，支持原子认领与租约              │
├─────────────────────────────────────────────────────────────────┤
│                      智能分析层                                  │
│  CodeExplorerAgent: 代码探索与语义分析（搜索、符号、调用关系）      │
│  DeepVulnAgent: 深度漏洞挖掘（Tool Call 驱动）                    │
│  AgentDelegate: 任务委托与 Agent 分发                            │
├─────────────────────────────────────────────────────────────────┤
│                      上下文管理层                                │
│  ContextAssembler: 消息组装与压缩触发                            │
│  ContextCompressor: 语义蒸馏 + Tool Call 去重                    │
│  MessageManager: 结构化消息管理与大文本落盘                       │
│  ArtifactStore: 统一存储任务输出与分析上下文                      │
├─────────────────────────────────────────────────────────────────┤
│                      引擎抽象层                                  │
│  (IDA Engine / JEB Engine / ABC Engine / Source Engine)         │
├─────────────────────────────────────────────────────────────────┤
│                      数据存储层                                  │
│  (Vuln Storage / Log Storage / SQLite Cache)                    │
└─────────────────────────────────────────────────────────────────┘
```

### Agentic 循环

核心执行流程由 LLM 自主决策，而非硬编码规则：
...
│                                                           │
│  ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌───────┐ │
│  │  Plan   │ →  │   Act   │ →  │ Review  │ →  │Append │ │
│  │规划任务 │    │执行任务 │    │检查结果 │    │或停止 │ │
│  └─────────┘    └─────────┘    └─────────┘    └───────┘ │
│       ↑                                            │     │
│       └────────────────────────────────────────────┘     │
│                                                           │
│  - Plan: LLM 根据 Workflow 文档规划任务                    │
│  - Act: TaskExecutorAgent 执行任务                        │
│  - Review: 检查执行结果，判断是否需要补充                   │
│  - Append/Stop: 追加新任务或结束循环                       │
└──────────────────────────────────────────────────────────┘
```
...
├── ivagent/                    # 核心包
│   ├── agents/                 # Agent 实现
│   │   ├── base.py             # Agent 基类（异步 LLM 交互、状态管理）
│   │   ├── code_explorer_agent.py  # 代码探索 Agent（搜索、符号、调用关系）
│   │   ├── deep_vuln_agent.py  # 深度漏洞挖掘 Agent（Tool Call 驱动）
│   │   ├── function_summary_agent.py  # 函数摘要 Agent
│   │   ├── callsite_agent.py   # 调用点识别 Agent
│   │   └── prompts.py          # 提示词模板
│   ├── orchestrator/           # 编排与工作流
│   │   ├── master_orchestrator.py  # 多 Workflow 协调器（Agentic 循环）
│   │   ├── orchestrator_agent.py   # 任务规划 Agent
│   │   ├── task_executor_agent.py  # 任务执行 Agent
│   │   ├── agent_delegate.py       # Agent 委托器
│   │   ├── task_list_manager.py    # SQLite 任务看板
│   │   ├── tools.py                # 编排工具集
...
│   │   └── context/            # 上下文管理（核心）
│   │       ├── context_compressor.py   # LLM 上下文压缩器
│   │       ├── context_assembler.py    # 上下文组装器
│   │       ├── message_manager.py      # 消息管理器
│   │       ├── artifact_store.py       # 统一存储
│   │       └── read_artifact_pruner.py # Artifact 修剪
│   ├── engines/                # 分析引擎
...
### 1. Agentic 循环 - LLM 自主驱动

系统核心是 **Plan → Act → Review → Append/Stop** 循环，由 LLM 自主决策：

```
┌──────────────────────────────────────────────────────────────┐
│  MasterOrchestrator                                          │
│                                                               │
│  1. Plan: 读取 Workflow 文档，规划任务列表                      │
│     └─→ plan_tasks(workflows) 输出结构化任务                   │
│                                                               │
│  2. Act: TaskExecutorAgent 执行任务                           │
│     └─→ delegate_task(agent_type) 分发到子 Agent              │
│                                                               │
│  3. Review: 检查执行结果                                       │
│     └─→ 判断是否需要补充信息或追加任务                          │
│                                                               │
│  4. Append/Stop:                                              │
│     ├─→ 有新发现 → append_tasks() 追加任务 → 回到 Act          │
│     └─→ 目标达成 → 结束循环                                    │
└──────────────────────────────────────────────────────────────┘
```
...
- LLM 自主决策何时追加任务、何时结束
- 支持多轮反思检查点
- 自动收敛判定（无 pending 任务 + 至少完成一个 vuln_analysis）
...
```
MasterOrchestrator (协调器)
    │
    ├── TaskExecutorAgent (任务执行器)
    │       │
...
| Agent | 职责 | 主要工具 |
|-------|------|----------|
| `code_explorer` | 代码探索与语义分析 | `search_symbol`, `get_function_def`, `get_callees`, `get_xrefs` |
| `vuln_analysis` | 深度漏洞挖掘 | `get_function_summary`, `create_sub_agent`, `report_vulnerability` |

### 3. Workflow 文本规划

使用 **Markdown 格式** 的 Workflow 文档描述分析意图，无需硬编码规则：
...
**关键机制**：
- **压缩前推理**：在压缩前让 LLM 进行推理并生成结构化摘要
- **Tool Call 去重**：防止重复调用相同参数的工具
- **大文本落盘**：超长内容存储到 ArtifactStore，消息中只保留引用
...
- 原子认领：多进程安全获取任务
- 租约机制：任务超时自动回收
- 幂等追加：基于 task_uid 唯一约束
- 依赖追踪：支持任务间依赖关系
```

### 6. DeepVulnAgent - Tool Call 驱动分析

深度漏洞挖掘 Agent，采用多轮对话和 Tool Call 机制：
...
## 发展路线图
- [x] Agentic 循环：Plan → Act → Review → Append/Stop
- [x] 上下文压缩：语义蒸馏 + Tool Call 去重
- [x] Workflow 文本规划：Markdown 驱动的分析意图描述
- [x] 多 Agent 协作：CodeExplorer + DeepVulnAgent + TaskExecutor
- [x] 统一任务看板：SQLite 持久化与原子认领
