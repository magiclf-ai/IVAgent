...
- `summary` 必须是可直接复用的高保真压缩摘要，且包含以下章节（无内容写“无”）：
  ...
  - `## 证据锚点`
  - `## 关键调用链`
- `summary` 还必须包含以下“最小补证判定”章节（无内容写“无”）：
  - `## 与当前分析目标匹配的语义理解蒸馏`
  - `### 中间结论`
  - `### 是否需要继续获取信息`
  - `### 最小补充信息集`
- `### 是否需要继续获取信息` 只能填写 `是` 或 `否`
  - 仅当缺失事实会直接阻塞当前下一步（例如第一次 `vuln_analysis` 或当前查询要求的直接输出）时，才能写 `是`
  - 若现有证据已经足以进入下一阶段，而剩余信息只是增强型 caller/xref/平行入口/外围覆盖补充，必须写 `否`
- `### 最小补充信息集` 只能列出真正阻塞当前下一步的最小缺口
  - 若不存在阻塞缺口，写 `无`
  - 禁止写成“继续全面枚举 caller/xref/所有入口/所有相邻函数”之类的泛化取证计划
...
class CodeExplorerAgent(BaseAgent):
    ...
    def finish_exploration(self, result: str, summary: str) -> str:
        ...
        - `summary` 还必须包含：
          ...
          - `### 是否需要继续获取信息`
          - `### 最小补充信息集`
        - 只有在“缺失事实会直接阻塞当前下一步”时，`### 是否需要继续获取信息` 才能写“是”
        - 若剩余信息只是增强型 caller/xref/外围覆盖补证，必须写“否”，并在 `### 最小补充信息集` 写“无”
...
8. `### 是否需要继续获取信息` 仅在“当前缺口会直接阻塞下一步”时填写“是”；若剩余仅为增强型补证，必须填写“否”
9. `### 最小补充信息集` 只能列阻塞当前下一步的最小缺口；若无阻塞缺口，写“无”
10. 在 result 中说明探索因迭代限制而终止
11. result 必须包含并完整填写以下章节：
   - ## 探索状态（完成 / 部分完成-迭代上限）
   - ## 当前可推断信息
   - ## 未推断信息
   - ## 需 Orchestrator 继续规划的 CodeExplorer 子任务"""
